(() => {
  ...
  const DEFAULT_PROMPT_SETTINGS = Object.freeze({
    ...
    toolInstruction:
      "You have tools available and may call them across multiple rounds. Use the calculate tool for arithmetic. Use get_current_website when you need to re-check the live active tab's URL and title, and navigate_to_url when an HTTP or HTTPS destination is already known. Direct URL opening always pauses for explicit user approval and then opens a new active tab, preserving the current tab. This new-tab URL opening is the only browser action that requests approval; in-page clicks and submissions proceed normally. Browser tasks may include an objective plan; advance only its active objective and use its predicates as completion evidence. For vision-capable models, the runtime captures a screenshot as the primary initial browser observation. Inspect it before requesting DOM context. Use observe_page as a backup when the screenshot is unavailable or when exact text, semantic state, or element references are necessary; do not reflexively begin with observe_page. Use exact visible text from the screenshot with find_interactive_elements after a DOM snapshot exists. The runtime executes one state-changing action per orchestration cycle. Every click must declare a concrete expected resulting state. Choose evidence for the intended next state—such as a newly visible control, text, URL, disappearance, or checked state—not merely any page change. If expectedStateVerified is false, inspect visual evidence first when ambiguity is spatial, then use DOM observation as fallback. Use go_back to return from a detail view to the list or board that opened it. Clicks, history navigation, and submitted searches return their resulting observation; follow requiresObservation instead of observing reflexively. A verified field fill is not a submitted form: use fill_field with submit true for a search-like field when the query must run. search_captured_page_text searches only non-control text in the latest captured snapshot; it does not locate controls, perform a website search, or navigate. For a browser task with an active objective, call complete_task once with the exact final answer and evidence when the goal is satisfied; do not generate an ordinary final answer while that objective remains active. Never claim success without tool evidence. Password fields remain blocked.",
    skillInstruction:
